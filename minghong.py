...
datas = []          
with open('C:/Users/李铭鸿/Desktop/train.csv','r') as input_file:
    reader = csv.reader(input_file)
    for row in reader:
        datas.append(row)
picture_names = [x[0] for x in datas[1:]]
picture_conditions = [(int(x[1]),int(x[2]),int(x[3])) for x in datas[1:]]

# 第一次遍历图片目录是为了获取图片总数
input_count = 0
dir = 'C:/Users/李铭鸿/Desktop/custom_images/'                 # 这里可以改成你自己的图片目录
...
for rt, dirs, files in os.walk(dir):
    for filename in files:
        ...
        img = Image.open(filename).convert("L").resize((28, 28),Image.ANTIALIAS)
        width = img.size[0]
        height = img.size[1]
...
with tf.Session() as sess:
    ...
    for it in range(iterations):
        ...
        for n in range(batches_count):
            train_step.run(feed_dict={x: input_images[n*batch_size:(n+1)*batch_size], y_: picture_conditions[n*batch_size:(n+1)*batch_size], keep_prob: 0.5})
        if remainder > 0:
            start_index = batches_count * batch_size
            train_step.run(feed_dict={x: input_images[start_index:input_count-1], y_: picture_conditions[start_index:input_count-1], keep_prob: 0.5})
 
        # 每完成五次迭代，判断准确度是否已达到100%，达到则退出迭代循环
        iterate_accuracy = 0
        if it%5 == 0:
            iterate_accuracy = accuracy.eval(feed_dict={x: input_images, y_: picture_conditions, keep_prob: 1.0})
